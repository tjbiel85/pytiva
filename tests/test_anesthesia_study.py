...
# will be used to load these data multiple times below
CSV_DICT_FOR_DATASETS = {
    'ds_cases': {'FILEPATH': os.path.join(testconfig.WD, testconfig.TESTDATA['DS_CASES'])},
    'ds_case_meds': {'FILEPATH': os.path.join(testconfig.WD, testconfig.TESTDATA['DS_CASE_MEDS'])},
    'ds_case_events': {'FILEPATH': os.path.join(testconfig.WD, testconfig.TESTDATA['DS_CASE_EVENTS'])},
}
...
class TestAnesthesiaStudy(unittest.TestCase):
    def test_study_from_csv_dict(self):
        ...
        datasets = datasets_from_csv_data(CSV_DICT_FOR_DATASETS)
